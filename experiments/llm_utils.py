"""
LLM Utilities for CUBE experiments (Qwen2.5-3B-Instruct).

Provides:
  - load_llm_model(lora_rank, device) → (model, tokenizer)
  - load_llm_dataset(name, split, n_pool) → list of dicts
  - build_llm_prompt(item) → str
  - extract_answer(text, qtype) → str
  - compute_reward(response, gold, qtype) → float
  - generate_rollouts_llm(model, tokenizer, items, N, ...) → Rollouts
  - generate_rollouts_llm_var(model, tokenizer, items, N, M, ...) → Rollouts (variable N_j)
  - compute_log_probs_batch(model, tokenizer, items, responses, device) → (M,) Tensor
...
Memory design:
  ...
  - generate_rollouts_llm: sequential per-rollout generation (variable-length).
"""
...
@dataclass
class Rollouts:
    """Container for a single (X, Y) LLM minibatch."""
    items: list               # B items (question/gold dicts)
...
def load_llm_model(lora_rank: int = 16, device: str = "cuda"):
    """Load a Qwen2.5-3B-Instruct model with LoRA adapter.

    Returns:
        model     : PeftModel with LoRA, in bf16
        tokenizer : AutoTokenizer for Qwen2.5
    """
...
def load_llm_dataset(name: str, split: str = None, n_pool: int = 512) -> list:
    """Load an LLM dataset and return list of standardized item dicts.

    Each item: {"question": str, "gold": str, "type": str}
    """
    import sys as _sys
    import os as _os
    ...
    items = []
    for ex in ds:
        question = str(ex.get(cfg["question_field"], ""))
        gold_raw = ex.get(cfg["gold_field"], "")
        
        # Handle gold field: may be string, int, or list
        if isinstance(gold_raw, list):
            gold = str(gold_raw[0])  # truthfulqa: take first correct answer
        else:
            gold = str(gold_raw)

        # For MCQ datasets: append choices to question if available
        if name in ("arc", "mmlu") and "choices" in ex:
            choices = ex["choices"]
            if isinstance(choices, dict):
                # MMLU format: {"A": "...", "B": "...", ...}
                choices_list = [choices.get(k, "") for k in sorted(choices.keys())]
            else:
                # ARC format: ["...", "...", ...]
...
def build_llm_prompt(item: dict) -> str:
    """Build Qwen2.5 chat prompt for tokenizer.apply_chat_template.

    Returns chat template string.
    """
    messages = [
        {
            "role": "user",
...
def generate_rollouts_llm(
    ...
    with torch.no_grad():
        for j, item in enumerate(items):
            messages = build_llm_prompt(item)
            text_prompt = tokenizer.apply_chat_template(
                messages, tokenize=False, add_generation_prompt=True
            )
            # Tokenize once per prompt
            inputs = tokenizer(
                text_prompt,
                return_tensors="pt",
                truncation=True,
...
def generate_rollouts_llm_var(
    ...
    Implements Adaptive Rollout Allocation for LLMs:
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. LLM Weighted Probe Projections (replaces compute_multi_weight_projs)
# ─────────────────────────────────────────────────────────────────────────────

def compute_llm_weight_projs(
    model,
    tokenizer,
    rollouts: Rollouts,
    ...
    """Compute R probe projections via n_w weighted backward passes for an LLM.
