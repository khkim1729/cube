"""
CUBE LLM Experiment Runner — single (baseline × budget) run on Qwen2.5-3B-Instruct.

Usage:
    # Dry run (1 step, no checkpoint):
    python experiments/run_llm.py --baseline rloo --budget none \\
        --dataset gsm8k --gpu_id 1 --num_train_steps 1 --T 0 --dry_run

    # Checkpoint-only (no training):
    python experiments/run_llm.py --baseline rloo --budget none \\
        --dataset gsm8k --gpu_id 1 --num_train_steps 0 --T 1 --S 2 --K 1

    # Full run:
    python experiments/run_llm.py --baseline rloo --budget none \\
        --dataset gsm8k --gpu_id 1
...
from experiments.llm_utils import (
    Rollouts,
    load_llm_model,
    load_llm_dataset,
    generate_rollouts_llm,
    generate_rollouts_llm_var,
    compute_llm_weight_projs,
)
...
# ─────────────────────────────────────────────────────────────────────────────
# build_D_B adapted for LLM Rollouts (same logic as cube_sim.build_D_B)
# ─────────────────────────────────────────────────────────────────────────────

def build_D_B_llm(rollouts: Rollouts, baseline: str, rewards: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
...
def measure_checkpoint_llm(
    ...
    """LLM version of cube_sim.measure_checkpoint.
    ...
    for s in range(S):
        ...
        for k in range(K):
            ...
            if budget == "rollout_alloc":
                rollouts = generate_rollouts_llm_var(
                    model, tokenizer, batch_items, N, M=B * N,
                    max_new_tokens=256, temperature=1.0, device=device,
                )
            else:
                rollouts = generate_rollouts_llm(
                    model, tokenizer, batch_items, N,
                    max_new_tokens=256, temperature=1.0, device=device,
                )
            r = rollouts.rewards  # (M,)

            # Build operators (same formulas as cube_sim)
            d_B = build_D_B_llm(rollouts, baseline, r)
            ...
            p1, p2, p3, p4, q = compute_llm_weight_projs(
                model, tokenizer, rollouts,
                [w1, w2, w3, w4, w5],
                R=R,
...
def train_step_llm(
    ...
    """One LLM policy gradient update step."""
    batch_items = random.sample(data_pool, min(B, len(data_pool)))
    if budget == "rollout_alloc":
        rollouts = generate_rollouts_llm_var(
            model, tokenizer, batch_items, N, M=B * N,
    ...
    from experiments.llm_utils import compute_log_probs_batch
...
def run_experiment(args):
    ...
    if args.dry_run:
        ...
        model.train()
        loss, rew, vr = train_step_llm(
            model, tokenizer, optimizer, data_pool,
...
def parse_args():
    p = argparse.ArgumentParser(description="CUBE LLM experiment runner (Qwen2.5-3B-Instruct)")
    # Experiment identity
    ...
    p.add_argument("--dataset",    type=str, default="gsm8k",
                   choices=["gsm8k", "arc", "mmlu", "truthfulqa"])
    p.add_argument("--gpu_id",     type=int, default=1)
    p.add_argument("--run_id",     type=str, default=None)
    p.add_argument("--output_dir", type=str, default="experiments/results_llm")
