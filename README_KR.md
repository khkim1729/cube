...
`run_batch.py`로 GPU 1,2,3에서 병렬 실행, 총 **309회 실험** (조합당 n=15–26회). 마지막 체크포인트(step 200) 기준.
...
|----------|------|:------:|:----------:|:-----------:|:------------:|
| REINFORCE | None | 26 | 0.000 | **0.000** | 3.91e-3 |
| REINFORCE | PromptSkip | 22 | 2.68e-3 | **0.000** | _2.60e-3_ |
| REINFORCE | RolloutAlloc | 21 | 2.28e-1 | **0.000** | 8.27e-3 |
| REINFORCE | SubsetSelect | 19 | 1.19e-1 | **0.000** | 7.81e-3 |
| GRPO | None | 16 | 1.47e-1 | 0.000 | 1.30e+13 |
| GRPO | PromptSkip | 19 | 1.61e-1 | 0.000 | 1.52e+12 |
| GRPO | RolloutAlloc | 20 | **4.16e-1** | **1.17e-2** | 1.89e+12 |
| GRPO | SubsetSelect | 21 | 3.57e-1 | 6.17e-3 | **2.59e+13** |
| RLOO | None | 21 | 3.20e-2 | 0.000 | 4.46e-3 |
| RLOO | PromptSkip | 15 | 3.12e-2 | _1.25e-4_ | 2.93e-3 |
| RLOO | RolloutAlloc | 20 | 2.16e-1 | 9.21e-3 | 9.13e-3 |
| RLOO | SubsetSelect | 20 | 1.01e-1 | 6.13e-3 | 8.93e-3 |
| STV (full) | None | 17 | 2.60e-2 | 0.000 | 4.07e-3 |
| STV (full) | PromptSkip | 19 | _2.54e-2_ | 8.22e-4 | 2.65e-3 |
| STV (full) | RolloutAlloc | 18 | 2.22e-1 | 6.30e-3 | 8.92e-3 |
| STV (full) | SubsetSelect | 15 | 9.77e-2 | 4.51e-3 | 8.13e-3 |

(굵은 글씨: 컬럼 최댓값, 이탤릭: 비GRPO 최솟값)

### 분석

**① Fusion Bias: 구조적 영조건 확인 (309회 전체)**

- **REINFORCE** ($A_B = 0$): 모든 budget 조합에서 Fusion Bias = 0.000 (309회 전체 확인)
- **None budget** ($H = H_0$): 모든 baseline 조합에서 Fusion Bias = 0.000 (309회 전체 확인)
- **DAPO-style PSkip**: $\text{Var}(r_j) = 0$인 그룹만 스킵하므로 $H \approx H_0$ → GRPO×PSkip Fusion ≈ 0
- Fusion Bias > 0 (큰 순서): GRPO×RAlloc(**1.17e-2**) > RLOO×RAlloc(9.21e-3) > GRPO×SubSel(6.17e-3) > RLOO×SubSel(6.13e-3) > STV×RAlloc(6.30e-3) > STV×SubSel(4.51e-3) >> STV×PSkip(8.22e-4) >> RLOO×PSkip(1.25e-4)
- **RolloutAlloc**이 SubsetSelect보다 큰 Fusion Bias — 비균일 배분이 $A_B$ 그룹 통계와 강하게 결합
- 309회까지 확장해도 순서 및 크기 **완전히 일치** — 결과 안정성 재확인

**② GRPO의 $\|HL\|_F^2$ 폭발**

| 구분 | HL proxy 범위 |
|------|--------------|
| GRPO 계열 | 1.52e+12 – 2.59e+13 |
| 비GRPO 계열 | 2.60e-3 – 9.13e-3 |

최소 GRPO HL / 최대 비GRPO HL ≈ **1.7 × 10¹⁴배** 차이 (309회에서도 동일).
원인: GRPO의 보상 의존적 $D_B$ (std 정규화)에서 $\sigma_\text{reward} \to 0$ 시 $D_B \to \infty$.
**DAPO PSkip 효과**: GRPO×None(1.30e+13) → GRPO×PSkip(1.52e+12). 분산=0인 그룹 필터링으로 HL **약 8.6배 감소**.
...
| PromptSkip | **2.60e-3** | **2.93e-3** | **2.65e-3** | **감소** ↓ (최저) |
| SubsetSelect | 7.81e-3 | 8.93e-3 | 8.13e-3 | **증가** ↑ |
| RolloutAlloc | 8.27e-3 | 9.13e-3 | 8.92e-3 | **더 증가** ↑↑ (최대) |

일관된 순서: **PromptSkip < None < SubsetSelect < RolloutAlloc**. 이는 v4에서 RolloutAlloc에 **실제 가변 N_j 샘플링**을 적용한 결과로, 이전 v3(PSkip < None < RAlloc < SubSel)과 순서가 달라졌다. 올바른 구현에서는 RolloutAlloc이 SubsetSelect보다 높은 HL proxy를 가짐.
...
| STV (full) | 2.60e-2 | James-Stein 적응 축소 적용 |
| RLOO | 3.20e-2 | LOO leave-one-out |
| GRPO | 1.47e-1 | $D_B$ 정규화 편향 |

STV가 RLOO보다 낮은 편향을 보이며, 두 방법 모두 GRPO 대비 현저히 낮음 (309회에서도 동일).

**⑤ RolloutAlloc vs SubsetSelect: Total Bias 비교 (309회)**

| 조합 | RAlloc Bias | SubSel Bias | 비율 |
|------|:-----------:|:-----------:|:----:|
| REINFORCE× | 2.28e-1 | 1.19e-1 | 1.9× |
| RLOO× | 2.16e-1 | 1.01e-1 | 2.1× |
| STV (full)× | 2.22e-1 | 9.77e-2 | 2.3× |

RolloutAlloc이 SubsetSelect보다 **1.9–2.3배 높은 Total Bias** (309회에서도 일관 확인).
실제 가변 N_j 배분에서 $H_j \propto$ Bernoulli 분산이 $H-H_0$ 편차를 더 극단적으로 만들어 예산 편향 증가.

**⑥ REINFORCE×PSkip: 비GRPO 조합 중 최저 HL (309회 재확인)**

REINFORCE×PSkip이 비GRPO 조합 중 최저 HL(2.60e-3)을 달성. DAPO-style 필터링이 A_B=0인 REINFORCE와 결합하여 Fusion Bias도 0으로 유지하면서 HL을 최소화.
STV×PSkip(2.65e-3)이 그 뒤를 이음 — REINFORCE×PSkip보다 Fusion Bias 8.22e-4만큼 추가 발생하지만 HL은 거의 동등.

**⑦ 결과 안정성 (264회 → 309회)**

45회 추가 실험 후 모든 주요 결론 유지. RolloutAlloc의 HL 순서 변화(v3 → v4)는 알고리즘 수정(실제 가변 N_j)에 의한 것으로, 같은 알고리즘 기준으로는 결과 완전히 안정적.
