...
`run_batch.py`로 GPU 1,2,3에서 병렬 실행, 총 **264회 실험** (조합당 n=24–43회). 마지막 체크포인트(step 180/200) 기준.
$v_r \sim \mathcal{N}(0,I)$ (비정규화 probe, **S=4, K=2** — 파일럿 프로토콜). DAPO-style PromptSkip 적용.

| 베이스라인 | 예산 | n_runs | Total Bias | Fusion Bias | $\|HL\|_F^2$ |
|----------|------|:------:|:----------:|:-----------:|:------------:|
| REINFORCE | None | 43 | 0.000 | **0.000** | 3.91e-3 |
| REINFORCE | PromptSkip | 35 | 3.37e-3 | **0.000** | **2.66e-3** |
| REINFORCE | RolloutAlloc | 33 | 2.46e-1 | **0.000** | 6.26e-3 |
| REINFORCE | SubsetSelect | 29 | 1.14e-1 | **0.000** | 7.81e-3 |
| GRPO | None | 28 | 1.35e-1 | 0.000 | 1.31e+13 |
| GRPO | PromptSkip | 29 | 1.53e-1 | 0.000 | 2.00e+12 |
| GRPO | RolloutAlloc | 31 | **4.55e-1** | **1.10e-2** | 2.10e+12 |
| GRPO | SubsetSelect | 33 | 3.42e-1 | 5.65e-3 | **2.58e+13** |
| RLOO | None | 33 | 3.06e-2 | 0.000 | 4.46e-3 |
| RLOO | PromptSkip | 24 | 2.97e-2 | 1.56e-4 | 2.98e-3 |
| RLOO | RolloutAlloc | 31 | 2.33e-1 | 1.01e-2 | 7.15e-3 |
| RLOO | SubsetSelect | 31 | 9.65e-2 | 5.58e-3 | 8.93e-3 |
| STV (full) | None | 25 | 2.46e-2 | 0.000 | 4.07e-3 |
| STV (full) | PromptSkip | 29 | _2.31e-2_ | 7.05e-4 | _2.69e-3_ |
| STV (full) | RolloutAlloc | 27 | 2.40e-1 | 5.34e-3 | 6.63e-3 |
| STV (full) | SubsetSelect | 24 | 9.43e-2 | 4.07e-3 | 8.13e-3 |

(굵은 글씨: 컬럼 최댓값, 이탤릭: 비GRPO 최솟값)

### 분석

**① Fusion Bias: 구조적 영조건 확인 (264회 전체)**

- **REINFORCE** ($A_B = 0$): 모든 budget 조합에서 Fusion Bias = 0.000 (264회 전체 확인)
- **None budget** ($H = H_0$): 모든 baseline 조합에서 Fusion Bias = 0.000 (264회 전체 확인)
- **DAPO-style PSkip**: $\text{Var}(r_j) = 0$인 그룹만 스킵하므로 $H \approx H_0$ → GRPO×PSkip Fusion ≈ 0
- Fusion Bias > 0 (큰 순서): GRPO×RAlloc(**1.10e-2**) > RLOO×RAlloc(1.01e-2) > GRPO×SubSel(5.65e-3) ≈ RLOO×SubSel(5.58e-3) > STV×RAlloc(5.34e-3) > STV×SubSel(4.07e-3) >> STV×PSkip(7.05e-4) >> RLOO×PSkip(1.56e-4)
- **RolloutAlloc**이 SubsetSelect보다 큰 Fusion Bias — 비균일 배분이 $A_B$ 그룹 통계와 강하게 결합
- 264회로 증가해도 순서 및 크기 **완전히 일치** — 결과 안정성 재확인

**② GRPO의 $\|HL\|_F^2$ 폭발**

| 구분 | HL proxy 범위 |
|------|--------------|
| GRPO 계열 | 2.00e+12 – 2.58e+13 |
| 비GRPO 계열 | 2.66e-3 – 8.93e-3 |

최소 GRPO HL / 최대 비GRPO HL ≈ **2.2 × 10¹⁴배** 차이 (264회에서도 동일).
원인: GRPO의 보상 의존적 $D_B$ (std 정규화)에서 $\sigma_\text{reward} \to 0$ 시 $D_B \to \infty$.
**DAPO PSkip 효과**: GRPO×None(1.31e+13) → GRPO×PSkip(2.00e+12). 분산=0인 그룹 필터링으로 HL **약 6.6배 감소**.
...
| PromptSkip | **2.66e-3** | **2.98e-3** | **2.69e-3** | **감소** ↓ (최저) |
| RolloutAlloc | 6.26e-3 | 7.15e-3 | 6.63e-3 | **증가** ↑ |
| SubsetSelect | 7.81e-3 | 8.93e-3 | 8.13e-3 | **더 증가** ↑↑ (최대) |

일관된 순서: **PromptSkip < None < RolloutAlloc < SubsetSelect**. 264회에서도 모든 비GRPO baseline에서 동일하게 재확인.
...
| STV (full) | 2.46e-2 | James-Stein 적응 축소 적용 |
| RLOO | 3.06e-2 | LOO leave-one-out |
| GRPO | 1.35e-1 | $D_B$ 정규화 편향 |

STV가 RLOO보다 낮은 편향을 보이며, 두 방법 모두 GRPO 대비 현저히 낮음 (264회에서도 동일).

**⑤ RolloutAlloc vs SubsetSelect: Total Bias 비교 (264회)**

| 조합 | RAlloc Bias | SubSel Bias | 비율 |
|------|:-----------:|:-----------:|:----:|
| REINFORCE× | 2.46e-1 | 1.14e-1 | 2.2× |
| RLOO× | 2.33e-1 | 9.65e-2 | 2.4× |
| STV (full)× | 2.40e-1 | 9.43e-2 | 2.5× |

RolloutAlloc이 SubsetSelect보다 **2.2–2.5배 높은 Total Bias** (264회에서도 일관 확인).
$H_j \propto$ 평균 보상 배분이 $H-H_0$ 편차를 더 극단적으로 만들어 예산 편향 증가.

**⑥ STV×PSkip: 비GRPO 조합 중 최저 HL (264회 재확인)**

STV×PSkip은 비GRPO 조합 중 최저 HL(2.69e-3)을 달성. DAPO-style 필터링이 James-Stein 적응 혼합과 시너지를 내어 분산 프록시를 최소화.
REINFORCE×PSkip(2.66e-3)과 거의 동등하나, Fusion Bias는 REINFORCE×PSkip=0 < STV×PSkip=7.05e-4.

**⑦ 결과 안정성 (219회 → 264회)**

45회 추가 실험 후 수치 변화가 모두 2번째 유효숫자 수준 이하 — 결론 및 순서 완전히 유지. 샘플 크기 충분성 확인.
