...
OUT_TRAIN      = "ceus_ap_c1_train.json"
OUT_VAL        = "ceus_ap_c1_val.json"
...
def to_cocovid(entries: List[Dict[str, Any]], include_folds: List[int]) -> Dict[str, Any]:
    ...
    for item in entries:
        ...
        # ap만 선택
        ap_idxs = [i for i, p in enumerate(phases) if str(p).upper() == "AP"]
        
        # 12장 검사
        assert len(ap_idxs) == 12, f"[{pid}] AP frames must be 12, got {len(ap_idxs)}"
        
        aug_flag_all = all(bool(is_aug_l[i]) for i in ap_idxs)
        ...
        for t, i in enumerate(ap_idxs):
            phase = phases[i]
            fname = fnames[i]
            bb    = bbox_str[i]
            aug_flag = bool(is_aug_l[i])
            ...
            images.append({
                ...
                "t1_min": t1_min[i] if i < len(t1_min) else None,
                "t1_sec": t1_sec[i] if i < len(t1_sec) else None
            })
    ...
    info_block = {
        "description": "CEUS liver dataset (1 Class: lesion, AP-only)",
